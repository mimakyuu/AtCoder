...
signed main() {
	...
	cin >> n;
	vb h(367, false);
	ll daycnt[] = {31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
	rep(i, 366) {
		if (i%7 == 0 || i%7 == 6) h[i] = true;
	}
	...
	rep(i, n) {
		...
		ll m = 0, d = 0;
		rep(i, v[0].size()) {
			m += (v[0][i]-'0') * pow(10, ((ll)v[0].size()-i-1));
		}
		rep(i, v[1].size()) {
			d += (v[1][i]-'0') * pow(10, ((ll)v[1].size()-i-1));
		}
