	...
	rrep(i, h) {
		...
		rep(j, h-i+1) {
			...
			rep(k, w) {
				if (k) rui[k] += rui[k-1];
				rep(l, i) {
					//debug(j+l+k, k, c[j+l][k]);
					if ((j+l+k) % 2 == 0) rui[k] += c[j+l][k];
					if ((j+l+k) % 2 == 1) rui[k] -= c[j+l][k];
				}
				//debug(rui[k]);
				
				if (rui[k] && pos[rui[k]+50000] != -1) chmax(tmp, k - pos[rui[k]+50000]);
				if (rui[k] && pos[rui[k]+50000] == -1) pos[rui[k]+50000] = k;
				if (!rui[k]) chmax(tmp, k - pos[50000]);
			}
			chmax(r, tmp*i);
		}
	}
