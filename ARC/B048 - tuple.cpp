...
signed main() {
	...
	map<ll, tuple<ll, ll, ll, ll, ll>> mp;
	
	ll bk = rh[0].first, cnt = 0, cbk = 0, c1 = 0, c2 = 0, c3 = 0;
	rep(i, n) {
		
		if (rh[i].first != bk) {
			mp[rh[i-1].first] = make_tuple(cbk, c1+c2+c3, c1, c2, c3);
			bk = rh[i].first, cbk = cnt, c1 = 0, c2 = 0, c3 = 0;
		}
		cnt++;
		if (rh[i].second == 1) c1++;
		if (rh[i].second == 2) c2++;
	...
	mp[rh[n-1].first] = make_tuple(cbk, c1+c2+c3, c1, c2, c3);
