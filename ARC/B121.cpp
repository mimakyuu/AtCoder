...
signed main() {
	...
	if (rc%2 == 0) {
		rep(i, 2*n) {
			if (pa[i].second == 'R') {
				pa[i].second = 'C';
			} else if (pa[i].second == 'G') {
				pa[i].second = 'A';
			} else if (pa[i].second == 'B') {
				pa[i].second = 'B';
			}
		}
	}
	...
	Sort(pa);
	ll r = LLINF;
	ll atmp = -1, btmp = -1, ctmp = -1;
	ll abtmp = LLINF, actmp = LLINF, bctmp = LLINF;
	rep(i, 2*n) {
		if (pa[i].second == 'A') {
			atmp = pa[i].first;
			if (btmp != -1) chmin(abtmp, atmp-btmp);
			if (ctmp != -1) chmin(actmp, atmp-ctmp);
		}
		if (pa[i].second == 'B') {
			btmp = pa[i].first;
			if (atmp != -1) chmin(abtmp, btmp-atmp);
			if (ctmp != -1) chmin(bctmp, btmp-ctmp);
		}
		if (pa[i].second == 'C') {
			ctmp = pa[i].first;
			if (atmp != -1) chmin(actmp, ctmp-atmp);
			if (btmp != -1) chmin(bctmp, ctmp-btmp);
		}
	}
	
	r = min(abtmp, actmp+bctmp);
	mes(r);
