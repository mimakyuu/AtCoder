...
signed main() {
    ...
    if (n%2 == 0) {
        ll r = 0;
        for (int i = 0; i < n/2 - 1; ++i) r += a[i] * 2;
        r += a[n/2 - 1];
        r -= a[n/2];
        for (int i = n/2 + 1; i < n; ++i) r -= a[i] * 2;
        mes(r);
    } else {
        ll r1 = 0;
        for (int i = 0; i < n/2 - 1; ++i) r1 += a[i] * 2;
        r1 += a[n/2 - 1] + a[n/2];
        for (int i = n/2 + 1; i < n; ++i) r1 -= a[i] * 2;
        
        ll r2 = 0;
        for (int i = 0; i < n/2; ++i) r2 += a[i] * 2;
        r2 -= a[n/2] + a[n/2 + 1];
        for (int i = n/2 + 2; i < n; ++i) r2 -= a[i] * 2;
        mes(max(r1, r2));
    }
