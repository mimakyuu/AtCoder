...
signed main() {
	...
	auto dist = [&](ll v, ll nv) {
		ll add = abs(x[nv] - x[v]) + abs(y[nv]- y[v]) + max(0ll, z[nv]- z[v]);
		return add;
	};
	
	vvll dp(1<<n, vll(n+1, LLINF));
	dp[1][0] = 0;
	...
	rep(bit, 1<<n) {
		rep(v, n) {
			...
			rep(nv, n) {
				...
				ll nbit = bit | (1<<nv);
				chmin(dp[nbit][nv], dp[bit][v] + dist(v, nv));
			}
		}
	}
