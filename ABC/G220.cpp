...
signed main() {
	...
	map<tuple<ll, ll, ll>, vector<tuple<ll, ll, ll>>> mp;
	rep(i, n) {
		rep(j, n) {
			if (i >= j) continue;
			ll a, b, d;
			a = 2*(x[j]-x[i]);
			b = 2*(y[j]-y[i]);
			d = x[j]*x[j] - x[i]*x[i] + y[j]*y[j] - y[i]*y[i];
			
			ll div = a;
			if (div == 0) div = b;
			if (div == 0) div = d;
			if (b != 0) div = gcd(div, b);
			if (d != 0) div = gcd(div, d);
			
			a /= div, b /= div, d /= div;
			mp[make_tuple(a, b, d)].pb(make_tuple(c[i]+c[j], x[i]+x[j], y[i]+y[j]));
			mp[make_tuple(-a, -b, -d)].pb(make_tuple(c[i]+c[j], x[i]+x[j], y[i]+y[j]));
		}
	}
	...
	for (auto i: mp) {
		rSort(i.second);
		ll tmp = 0;
		ll ux = -LLINF, uy = -LLINF;
	
		rep(j, i.second.size()) {
			if (ux != get<1>(i.second[j]) || uy != get<2>(i.second[j])) {
				tmp += get<0>(i.second[j]);
				if (ux != -LLINF && uy != -LLINF) {
					chmax(r, tmp);
					break;
				}
				ux = get<1>(i.second[j]);
