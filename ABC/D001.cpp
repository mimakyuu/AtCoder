...
signed main() {
    ...
    rep(i, n) {
        string s;
        cin >> s;
        ll start = atoi(s.substr(0, 4).c_str());
        start = start - (start%5);
        ll end = atoi(s.substr(5).c_str());
        if (end % 5 != 0) {
            end = end + 5 - (end%5);
        }
    ...
    if(rain[0] == 1) {
        cout << setfill('0') << right << std::setw(4) << 0 << "-";
    }
    ...
    for (ll t = 1; t <= 2401; t++) {
        ...
        if (rain[t-1] == 0 && rain[t] == 1) {
            cout << setfill('0') << right << std::setw(4) << t << "-";
        }
        if (rain[t-1] == 1 && rain[t] == 0) {
            cout << setfill('0') << right << std::setw(4) <<  t-1 << endl;
        }
    }
