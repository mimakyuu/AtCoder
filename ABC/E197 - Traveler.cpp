...
signed main() {
	...
	rrep(i, m) {
		...
		if (x <= ma[i]) {
			...
			chmin(dp[i][0], dp[i-1][0] + (x-mi[i]));
		}
		
		if (x >= mi[i]) {
			chmin(dp[i][1], dp[i-1][0] + (x-mi[i]) + (ma[i]-mi[i]));
		} else {
			chmin(dp[i][1], dp[i-1][0] + (ma[i]-x));
		}
		
		x = ma[i-1];
		if (x <= ma[i]) {
			chmin(dp[i][0], dp[i-1][1] + (ma[i]-x) + (ma[i]-mi[i]));
		} else {
			chmin(dp[i][0], dp[i-1][1] + (x-mi[i]));
		}
		if (x >= mi[i]) {
			chmin(dp[i][1], dp[i-1][1] + (x-mi[i]) + (ma[i]-mi[i]));
		} else {
