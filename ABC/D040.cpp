...
signed main() {
	...
	rep(i, q) {
		...
		cin >> v >> w;
		v--;
		vec.pb(make_tuple(w, i+1000000, v));
	}
	rSort(vec);
	vll size(n, 1); //size[tree.root(x)]が連結成分の大きさを保持
	vll r(q);
	rep(i, m+q) {
		ll v = get<2>(vec[i]);
		if (get<1>(vec[i]) >= 1000000) {
			r[get<1>(vec[i])-1000000] = size[tree.root(v)];
			
		} else {
			ll a = get<1>(vec[i]), b = get<2>(vec[i]);
			
            //連結成分の大きさを更新
			if (tree.root(a) != tree.root(b)) {
				ll tmp = size[tree.root(a)] + size[tree.root(b)];
				tree.merge(a, b);
				size[tree.root(a)] = tmp;
			}
