...
signed main() {
    ...
    for (ll i=2; i < n; i += 2) {
        lsum[i] = lsum[i-2] + h[i-1] - h[i-2];
        
        rsum[i] = rsum[i-2] + h[n-i+1] - h[n-i];
    }
    
    
    rep(i, m) {
        ll p = lower_bound(h.begin(), h.end(), w[i]) - h.begin();
        ll tmp;
        
        if (p % 2 == 0) {
            tmp = lsum[p] + rsum[n-p-1] + h[p] - w[i];
        } else {
            tmp = lsum[p-1] + rsum[n-p] + w[i] - h[p-1];
        }
