...
signed main() {
    ...
    ll now = 1; //今tkhsくんがいる場所
    ll cnt = 0; //何手目か
    map <ll, ll> mp; //この町に来るのは何回目ですか
    ll loop = 0; //何回のTeleportで周回するか
    vll lp(n); //ループ順格納配列
    ll flg = 0; //ループに入ったかフラグ
    ...
    while(true) { //O(2n)
        ...
        if (mp[now] == 2) flg = 1; //2回目に訪れたらループ計測開始
        if (mp[now] == 3) break; //3回目:ループ1順
        if (flg == 1) {
            lp[loop] = now;
            loop++;
        }
