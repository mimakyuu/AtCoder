...
struct SegTree {
    ...
    SegTree(int n, const F func, const T &init) : f(func), INIT(init) {
        size = 1;
        while(size < n) size <<= 1;
        dat.assign(2*size, INIT);
    }
...
signed main() {
    ll n, q;
    cin >> n >> q;
    auto func = [] (ll x, ll y) {return x ^ y;}; //seg.query()でこれを計算するよ
    SegTree<ll> seg(n, func, 0); //要素数,func,初期値
    rep(i, n) {
        ll a;
        cin >> a;
        seg.set(i, a); //i番目の要素をaに設定
    }
    ...
    rep(i, q) {
        ...
        if (t == 1) {
            x--;
            seg.update(x, seg[x] ^ y); //x番目の要素を更新
        } else {
            x--; y--;
            mes(seg.query(x, y)); //x番目からy番目の範囲で計算
        }
