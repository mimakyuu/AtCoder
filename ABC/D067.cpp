...
signed main() {
	...
	while (!que.empty()) {
		...
		for (auto nv : g[v]) {
			if (dist_f[nv] != -1) continue;
			dist_f[nv] = dist_f[v] + 1;
			que.push(nv);  
	...
	ll mi = LLINF;
	rep(i, n) {
		chmin(mi, dist_s[i]+dist_f[i]);
		//debug(dist_s[i], dist_f[i], dist_s[i]+dist_f[i]);
		
	}
	
	rep(i, n) {
		if (dist_s[i]+dist_f[i] == mi && dist_f[i] <= dist_s[i]) {
			dist_s[i] = LLINF;
		} else {
			dist_s[i] = -1;
		}
	...
	(cnt < n-cnt) ? mes("Fennec") : mes("Snuke");
