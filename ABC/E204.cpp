...
struct graph{
  	...
  	void add_edge(ll s, ll t, ll cost, ll d){
    	edge e;
    	e.to = t, e.cost = cost;
		e.d = d;
    	G[s].push_back(e);
    	...
    	while (!que.empty()){
			...
			que.pop();
      		...
      		for (auto e : G[v]){
				...
				ll start = round(sqrt(e.d))-1;
        		if (d[e.to] > max(d[v], start)+e.cost+(e.d/(max(d[v], start)+1)) ){
          			d[e.to] = max(d[v], start)+e.cost+(e.d/(max(d[v], start)+1));
